...
//--------------*CLASSES AND METHODS----------------------------------------------------
	...
	class Node{
		constructor(x, y, w, h, isDragging, inputHandleFill, outputHandleFill){
			...
			this.inputHandleFill = typeof inputHandleFill !== 'undefined' ? inputHandleFill : HANDLE_COLOR;
		...
		handleSelected(x, y){
			if(Math.sqrt((x-this.x)*(x-this.x) + 
				(y1-Math.floor(this.y + (this.h)/2))*(y1-Math.floor(this.y + (this.h)/2))) < HANDLE_RADIUS){
				return "input";
			}
