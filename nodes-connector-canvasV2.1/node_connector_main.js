window.addEventListener("load", () => {
	...
	const excelMenu = document.querySelector("#excelsmenu");
	const pdfMenu = document.querySelector("#pdfsmenu");
	const sapMenu = document.querySelector("#sapsmenu");
...
//--------------*CLASSES AND METHODS----------------------------------------------------
	...
	function nodesMenuClick(e){
		...
		nodesArray.push(new Node(NEW_NODE_X, NEW_NODE_Y, 100, 70, false, false, false, undefined, undefined,
								 e.target.text, seqName.concat(nodesArray.length + 1)));
		nodesArray[nodesArray.length - 1].create(ctx);
		if (nodesArray.length % 3 != 0) {
			// if (NEW_NODE_Y > 449) {
			// 	NEW_NODE_Y = 150;
			// }
			NEW_NODE_X = NEW_NODE_X + 250;
		}
		else{
			NEW_NODE_X = 150;
			NEW_NODE_Y = NEW_NODE_Y + 150;
		}
...
//--------------CLASSES AND METHODS*----------------------------------------------------
	...
	pdfMenu.addEventListener("click", nodesMenuClick, false);
	sapMenu.addEventListener("click", nodesMenuClick, false);
